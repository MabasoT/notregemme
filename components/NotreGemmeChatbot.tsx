...
function getBotReply(input: string): string {
  ...
  // Track pants / tracksuit
  if (/\btrack\s*pant|\btracksuit|\bpant\b/.test(q)) {
        return "Track Pants are currently available for **pre-order** 🔥 Pricing will be confirmed on order. Drop us a WhatsApp to lock yours in!";
  }

  // Pre-order
  if (/\bpre[\-\s]?order\b/.test(q)) {
        return "Currently, **Track Pants** are available for pre-order. Contact us on WhatsApp to place yours and we'll confirm sizing and pricing. More items may be added — check back soon!";
  }
  ...
  if (/\bcontact|\blocation|\bwhere|\baddress|\bphone|\bnumber\b/.test(q)) {
        return `You can reach us on WhatsApp or Instagram (@${siteConfig.brand.name.toLowerCase().replace(/\s/g, "")}). We're based in South Africa 🇿🇦`;
  }
...
export function NotreGemmeChatbot(): React.ReactElement {
  ...
  function renderText(text: string) {
        ...
        return parts.map((part, i) =>
                i % 2 === 1 ? <strong key={i}>{part}</strong>strong> : <span key={i}>{part}</span>span>,
                             );
  }
        ...
        <>
          {/* Floating toggle button */}
              <button
                        ...
                        className="fixed bottom-6 right-6 z-[300] flex h-14 w-14 items-center justify-center rounded-full shadow-2xl transition-transform duration-200 hover:scale-105 active:scale-95 sm:bottom-8 sm:right-8"
                        style={{
          ...
          {open && (
                  <div
                              ...
                              className="fixed bottom-24 right-4 z-[299] flex w-[calc(100vw-2rem)] max-w-[380px] flex-col overflow-hidden rounded-2xl shadow-2xl sm:bottom-28 sm:right-8"
                              style={{
